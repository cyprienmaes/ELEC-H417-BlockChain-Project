                ...
                if data:
                    ...
                    try:
                        ...
                        if receivedBlock in self.blockchain.waiting_blocks:
                            if addr in self.confirmed:
                                continue
                            else:
                                self.confirmed.append(addr)
                                
                            if self.verifyConfirmed:
                                print('coucou')
                        ...
                        elif receivedBlock == self.blockchain.last_block() and self.blockchain != []:
                            pass
                        
                        else :
                            #print(receivedBlock['index'])
...
##                        else:
##
####                          proof, time_proof = node.blockchain.proof_of_work(node.blockchain.last_block())
####                          previous_hash = hashBlock(node.blockchain.last_block())
##                            proof, time_proof = node.blockchain.proof_of_work()
##                            previous_hash = hashBlock(node.blockchain.last_block())
##                            #blockchain.new_block(blockchain.last_block['hash'])
##                            node.blockchain.new_block(amount, proof, time_proof, previous_hash)
##                            newBlock = node.blockchain.new_block(amount, proof, time_proof, previous_hash)
##                            nodesMessage = Thread(target = node.runNodesMessage) #Problem. We kill the last thread even if it didn't accomplished the task
##                            node.message = node.setMessage({'Block':newBlock})
##                            nodesMessage.start()
